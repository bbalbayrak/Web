...
exports.updateWork = async (req, res) => {
    try {
      const workId = req.params.id;
      const { order_number, project_number, vendor_id, customer_id, quality_responsible_id, inspector_id, foreman_id, work_type, state, status, creator_name, creation_date, product_id } = req.body;
      const updatedWork = await Work.update(workId, order_number, project_number, vendor_id, customer_id, quality_responsible_id, inspector_id, foreman_id, work_type, state, status, creator_name, creation_date, product_id);
      if (!updatedWork) {
        res.status(404).send({ message: "Work not found" });
        return;
      }
